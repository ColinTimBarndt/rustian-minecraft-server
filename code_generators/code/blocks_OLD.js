...
let writer = fs
  .createWriteStream("generated_code/blocks.rs", { flags: "w" })
  .on("finish", () => {
...
const WOOD_TYPES = Object.entries(data)
  ...
  .map(([name, block]) =>
    (name = parseKey(name).id).substring(0, name.length - "_planks".length)
  );
const COLORS = Object.entries(data)
  .filter(([name, block]) => parseKey(name).id.endsWith("_stained_glass_pane"))
  .map(([name, block]) =>
    (name = parseKey(name).id).substring(
      0,
      name.length - "_stained_glass_pane".length
    )
  );
...
for (let blockId in data) {
  ...
  if (blocksWithData.has(key.id)) {
    let stateName = similarStructures.has(key.id)
      ? toCamelCase(similarStructures.get(key.id))
      : camelCaseName;
    stateName += "Data";
    ...
    writer.write(
      Object.entries(properties)
        .map(([propName, propVal]) => {
          ...
          return `${
            propName == "type" ? "r#type" : propName
          }: ${sProp.createValue(propVal)}`;
        })
        .join(", ")
    );
    writer.write("}),");
...
function tryUseNativeType(propValues) {
  switch (propValues.length) {
    ...
    case 3:
      if (
        propValues.includes("x") &&
        propValues.includes("y") &&
      ...
      if (
        propValues.includes("floor") &&
        propValues.includes("wall") &&
      ...
      if (
        propValues.includes("single") &&
        propValues.includes("left") &&
      ...
      if (
        propValues.includes("up") &&
        propValues.includes("side") &&
    ...
    case 4:
      if (
        propValues.includes("north") &&
        propValues.includes("south") &&
        propValues.includes("west") &&
        propValues.includes("east")
      ) {
        return {
          map: {
            north: "North",

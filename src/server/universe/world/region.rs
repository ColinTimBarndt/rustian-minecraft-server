...
impl Region {
    ...
    pub fn get_chunk_at_offset_mut(&mut self, offset: ChunkPosition) -> Option<&mut Chunk> {
    ...
    async fn handle_message(&mut self, message: <Self::Handle as ActorHandle>::Message) -> bool {
        match message {
            ...
            RegionMessage::GetBlock {
                ...
                {
                    let chk_pos = ChunkPosition::from(off);
                    if let Some(chunk) = self.get_chunk_at_offset(chk_pos) {
                        send_block(
                            send,
            ...
            RegionMessage::SetBlock { offset: off, block } => {
                if (0..REGION_COORDINATE_SCALE).contains(&off.x)
                    && (0..256).contains(&off.y)
                    && (0..REGION_COORDINATE_SCALE).contains(&off.z)
                {
                    let chk_rel_pos = ChunkPosition::from(off);
                    if let Some(chunk) = self.get_chunk_at_offset_mut(chk_rel_pos) {
                        let chk_abs_pos = chunk.position;
                        let inner_offset = Vec3d::new(off.x % 16, off.y, off.z % 16);
                        ...
                        let subscribers = &mut self.subscribers
                            [Region::get_chunk_index_from_offset(&chk_rel_pos)];
                        let packet = crate::packet::play::send::BlockChange::new(
                            chk_abs_pos.get_offset() + inner_offset,
                            block,
                        );
                        let handles = subscribers
                            .iter_mut()
                            .map(|(_, subscriber)| subscriber.send_packet(packet));
            ...
            RegionMessage::BroadcastChunk { offset, sender } => {
                ...
                #[inline]
                fn send_chunk(
                    send: broadcast::Sender<Option<Arc<(u32, Vec<u8>)>>>,
                    b: Option<ChunkData>,
                ) {
...
impl RegionHandle {
    ...
    async fn perform_tick(&mut self, amount: u8) -> ActorMessagingResult {
        self.send_raw_message(ActorMessage::Other(RegionMessage::PerformTick(amount)))
            .await
    }
    ...
    pub async fn get_block(&mut self, offset: Vec3d<i32>) -> ActorMessagingResult<Option<Block>> {
        ...
        self.send_raw_message(ActorMessage::Other(RegionMessage::GetBlock {
            offset,
            channel: sender,
        }))
        .await?;
    ...
    pub async fn set_block(&mut self, offset: Vec3d<i32>, block: Block) -> ActorMessagingResult {
        self.send_raw_message(ActorMessage::Other(RegionMessage::SetBlock {
            offset,
            block,
        }))
        .await
    ...
    pub async fn set_chunk(&mut self, chunk: Box<Chunk>) -> ActorMessagingResult {
        self.send_raw_message(ActorMessage::Other(RegionMessage::SetChunk(chunk)))
            .await
    }
    ...
    pub async fn player_subscribe(
        ...
        self.send_raw_message(ActorMessage::Other(RegionMessage::PlayerSubscribe {
            chunk,
            player_id,
            connection,
            send_complete,
    ...
    pub async fn player_unsubscribe(
        ...
        self.send_raw_message(ActorMessage::Other(RegionMessage::PlayerUnsubscribe {
            chunk,
            player_id,
        }))
        .await

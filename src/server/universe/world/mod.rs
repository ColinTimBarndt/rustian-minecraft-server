...
use crate::helpers::{NibbleArray4096, BitArray, Vec3d};
...
where Generator: ChunkGenerator, Loader: ChunkLoader {
...
// https://github.com/feather-rs/feather/blob/develop/core/src/world/chunk.rs
...
impl ChunkSection {
    ...
    pub fn set_block_state_at(&mut self, pos: usize, state: usize) {
        ...
        let palette_index: usize = if let Some(palette) = self.palette.as_mut() {
            ...
            match palette.binary_search(&(state as u16)) {
                Ok(index) => index,
                Err(index) => { // Or add it to the palette
                    palette.insert(index, state as u16);
                    // Resize?
                    let bpb = (64 - (palette.len()-1).leading_zeros()) as u8;
                    let palette_index = if bpb <= BitArray::MAX_BITS_PER_ITEM {
                        self.data = self.data.resize_to(bpb).unwrap();
                        index
                    } else {
                        // Use global palette
                        let mut new_data = BitArray::new(block::blocks::USED_PALETTE_BITS, 16*16*16);
                        for pos in 0..16*16*16 {
        ...
        if old_state == air_state {
            if state != air_state { // Replaced air with solid
                self.solid_blocks += 1;
            }
        } else {
            if state == air_state { // Replaced solid with air
                self.solid_blocks -= 1;
            }
        }
    ...
    pub fn optimize(&mut self) -> bool {
        ...
        for pos in 0..16*16*16 {
            let block = self.get_block_state_at(pos) as u16;
            self.data.set(pos, new_palette.binary_search(&block).unwrap() as u64);
        }

        let mut optimized_bpb = (64 - (new_palette.len()-1).leading_zeros()) as u8;

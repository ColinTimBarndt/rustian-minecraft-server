...
impl UniverseHandle {
    pub async fn create_world(
        ...
        self.send_raw_message(ActorMessage::Other(UniverseMessage::CreateWorld {
            key: id,
    ...
    pub async fn get_world(
        ...
        let (send, recv) = oneshot::channel();
        self.send_raw_message(ActorMessage::Other(UniverseMessage::GetWorld(id, send)))
            .await?;
        Ok(recv.await?)
    }
    ...
    pub async fn join_world(
        ...
        let (send, recv) = oneshot::channel();
        self.send_raw_message(ActorMessage::Other(UniverseMessage::JoinWorld(
            player, send,
        )))
        .await?;
        Ok(recv.await?)
    ...
    pub async fn reserve_entity_id(&mut self) -> ActorMessagingResult<u32> {
        let (send, recv) = oneshot::channel();
        self.send_raw_message(ActorMessage::Other(UniverseMessage::ReserveEntityId(send)))
            .await?;
        Ok(recv.await?)
    }
    pub async fn free_entity_id(&mut self, id: u32) -> ActorMessagingResult {
        self.send_raw_message(ActorMessage::Other(UniverseMessage::FreeEntityId(id)))
            .await
    }
}
